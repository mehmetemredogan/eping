...
project's **next planned release** (currently `0.1.4`). It follows a plain
`MAJOR.MINOR` (or `MAJOR.MINOR.PATCH`) scheme — no leading `v`, no trailing
newline content beyond the number itself.

To cut a new release:

1. Bump `ui/VERSION` (e.g. `0.1.3` → `0.1.4`) in a commit/PR.
2. Either push a matching tag (`git tag v0.1.4 && git push origin v0.1.4`), or
